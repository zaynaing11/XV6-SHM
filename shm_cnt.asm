...
shm_open(1,(char **)&counter);
    ...
    1076:	c7 44 24 04 44 18 00 	movl   $0x1844,0x4(%esp)
    107d:	00 
  ...
  if(pid)
    10ca:	74 2a                	je     10f6 <main+0xf6>
       printf(1," Final Counter in parent is %d\n",counter->cnt);
    10cc:	c7 44 24 04 68 18 00 	movl   $0x1868,0x4(%esp)
    10d3:	00 
...
//shm_close: first process will just detach, next one will free up the shm_table entry (but for now not the page)
   ...
   exit();
    ...
    printf(1,"Final Counter in child is %d\n\n",counter->cnt);
    10f6:	c7 44 24 04 88 18 00 	movl   $0x1888,0x4(%esp)
    10fd:	00 
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    1429:	0f b6 92 af 18 00 00 	movzbl 0x18af(%edx),%edx
  }while((x /= base) != 0);
    ...
    149e:	8d 45 10             	lea    0x10(%ebp),%eax
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    15bf:	8b 38                	mov    (%eax),%edi
          s = "(null)";
    15c1:	b8 a8 18 00 00       	mov    $0x18a8,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1681:	a1 6c 1b 00 00       	mov    0x1b6c,%eax
{
  ...
  freep = p;
    16c9:	a3 6c 1b 00 00       	mov    %eax,0x1b6c
}
    16ce:	5b                   	pop    %ebx
    16cf:	5e                   	pop    %esi
    16d0:	5f                   	pop    %edi
  ...
  freep = p;
    16f2:	a3 6c 1b 00 00       	mov    %eax,0x1b6c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    171c:	8b 1d 6c 1b 00 00    	mov    0x1b6c,%ebx
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    175b:	3b 15 6c 1b 00 00    	cmp    0x1b6c,%edx
  ...
  return freep;
    1792:	8b 15 6c 1b 00 00    	mov    0x1b6c,%edx
    ...
    17b4:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    17b7:	89 15 6c 1b 00 00    	mov    %edx,0x1b6c
      return (void*)(p + 1);
    ...
    base.s.ptr = freep = prevp = &base;
    17ce:	c7 05 6c 1b 00 00 70 	movl   $0x1b70,0x1b6c
    17d5:	1b 00 00 
    base.s.size = 0;
    17d8:	ba 70 1b 00 00       	mov    $0x1b70,%edx
    base.s.ptr = freep = prevp = &base;
    17dd:	c7 05 70 1b 00 00 70 	movl   $0x1b70,0x1b70
    17e4:	1b 00 00 
    base.s.size = 0;
    17e7:	c7 05 74 1b 00 00 00 	movl   $0x0,0x1b74
    17ee:	00 00 00 
